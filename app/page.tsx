  ...
  return (
    ...
    <div className="min-h-screen w-full max-w-[100vw] bg-slate-50 font-sans text-slate-900 overflow-x-hidden selection:bg-yellow-500 selection:text-slate-900 relative">
      ...
      <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled || isMenuOpen ? 'bg-slate-900/95 backdrop-blur-md shadow-xl py-3' : 'bg-transparent py-4 md:py-6'}`}>
        <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center">
            <a href="tel:+36306457041" className="flex items-center gap-3 px-6 py-3 bg-yellow-500 hover:bg-yellow-400 text-slate-900 font-black text-sm uppercase tracking-wide rounded-lg transition-transform transform hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(234,179,8,0.5)]">
        ...
        <div className={`fixed inset-x-0 top-[60px] bg-slate-900/95 backdrop-blur-xl border-b border-slate-800 overflow-hidden transition-all duration-300 ease-in-out lg:hidden ${isMenuOpen ? 'max-h-screen opacity-100 py-4 shadow-2xl' : 'max-h-0 opacity-0 py-0'}`}>
          <div className="container mx-auto px-4 flex flex-col gap-2">
             {['Szolgáltatások', 'Referenciák', 'Kapcsolat'].map((item) => (
              <a 
                key={item} 
                href={`#${item.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")}`} 
                className="text-white text-lg font-bold uppercase tracking-wide flex justify-between items-center py-4 border-b border-slate-800/50 hover:text-yellow-500 hover:pl-2 transition-all"
                onClick={() => setIsMenuOpen(false)}
              >
                {item} <ChevronRight size={18} className="text-yellow-500/50"/>
              </a>
            ))}
            <a href="tel:+36306457041" className="mt-6 flex justify-center items-center gap-3 w-full py-4 bg-yellow-500 text-slate-900 font-black uppercase rounded shadow-lg active:scale-95 transition-transform">
      ...
      */}
        ...
        <div className="container mx-auto px-4 relative z-10 text-white">
          <RevealOnScroll>
            <div className="max-w-5xl mx-auto lg:mx-0 text-center lg:text-left">
              ...
              <p className="text-base md:text-xl lg:text-2xl text-slate-200 mb-8 md:mb-12 max-w-2xl mx-auto lg:mx-0 font-medium leading-relaxed drop-shadow-md bg-slate-900/40 p-4 rounded-xl backdrop-blur-sm border border-white/5">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                <a href="#kapcsolat" className="group px-8 py-4 md:px-10 md:py-5 bg-yellow-500 hover:bg-yellow-400 text-slate-900 font-black text-lg md:text-xl uppercase tracking-wide rounded shadow-[0_0_20px_rgba(234,179,8,0.3)] hover:shadow-[0_0_30px_rgba(234,179,8,0.6)] flex items-center justify-center gap-3 transition-all hover:-translate-y-1">
                  Ingyenes Felmérés <ArrowRight className="group-hover:translate-x-1 transition" size={24} />
                </a>
                <a href="#referenciak" className="px-8 py-4 md:px-10 md:py-5 border-2 border-slate-500 hover:border-yellow-500 text-white hover:text-yellow-500 font-bold text-lg md:text-xl uppercase tracking-wide rounded transition-all text-center bg-slate-900/60 backdrop-blur-sm hover:bg-slate-900/80">
                  Munkáink
                </a>
              </div>
            </div>
      ...
      {/* --- SZOLGÁLTATÁSOK --- */}
      <section id="szolgaltatasok" className="py-16 md:py-24 lg:py-32 bg-slate-50 relative">
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8 lg:gap-10">
            ...
            <ServiceCard delay={0} icon={<ShieldCheck />} title="Térkőágyazat" desc="Térkő alatti rétegrend (tükör) precíz kialakítása és tömörítése." />
            <ServiceCard delay={100} icon={<CheckCircle />} title="Tükörszedés" desc="A talaj felső rétegének eltávolítása és lézeres szintezése." />
      ...
      {/* --- GÉPPARK --- */}
      <section id="geppark" className="py-16 md:py-24 lg:py-32 bg-slate-950 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          {/* Reszponzív Bento Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 md:gap-6 lg:min-h-[600px]">
            
            <div className="lg:col-span-8 relative group rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-slate-800 h-64 md:h-80 lg:h-auto">
               <RevealOnScroll className="h-full w-full">
                 ...
                 <div className="absolute bottom-0 left-0 p-6 md:p-10">
                    ...
                    <h3 className="text-white text-2xl md:text-4xl lg:text-5xl font-black uppercase italic mb-1">Caterpillar Flotta</h3>
                 </div>
               </RevealOnScroll>
            </div>

            <div className="lg:col-span-4 relative group rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-slate-800 h-64 md:h-80 lg:h-auto">
               <RevealOnScroll delay={100} className="h-full w-full">
                 ...
                 <div className="absolute bottom-0 left-0 p-6">
                    <div className="inline-block px-3 py-1 mb-2 bg-blue-500 text-white text-[10px] md:text-xs font-black uppercase rounded">Precizitás</div>
                    <h3 className="text-white text-xl md:text-2xl font-bold uppercase">Mini Kotró</h3>
                 </div>
               </RevealOnScroll>
            </div>

            <div className="lg:col-span-6 relative group rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-slate-800 h-64 lg:h-80">
               <RevealOnScroll delay={200} className="h-full w-full">
                 ...
                 <div className="absolute bottom-0 left-0 p-6">
                    ...
                    <h3 className="text-white text-xl md:text-2xl font-bold uppercase">Teherautó Park</h3>
                 </div>
               </RevealOnScroll>
            </div>
            ...
            <div className="lg:col-span-6 relative group rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-slate-800 h-64 lg:h-80">
               <RevealOnScroll delay={300} className="h-full w-full">
                 ...
                 <div className="absolute bottom-0 left-0 p-6">
                    ...
                    <h3 className="text-white text-xl md:text-2xl font-bold uppercase">Gabonaszállítás</h3>
                 </div>
               </RevealOnScroll>
            </div>
      ...
      {/* --- GALÉRIA --- */}
      <section id="referenciak" className="py-16 md:py-24 lg:py-32 bg-slate-900 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <RevealOnScroll>
            {/* Filter */}
            <div className="flex flex-nowrap md:flex-wrap overflow-x-auto md:overflow-visible gap-3 mb-8 md:mb-16 pb-4 md:pb-0 scrollbar-hide px-2 -mx-2 md:justify-center">
              {categories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setFilter(cat)}
                  className={`flex-shrink-0 px-5 py-2.5 rounded-full font-bold uppercase text-xs tracking-wider transition-all duration-300 border whitespace-nowrap ${
                    filter === cat 
              ...
              <RevealOnScroll key={`${item.src}-${idx}`} delay={(idx % 4) * 50}>
                ...
                >
                  <img 
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-90 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 md:p-6">
                    <span className="text-yellow-400 text-[10px] font-black uppercase tracking-widest mb-1">{item.cat}</span>
                    <h4 className="text-white font-bold text-sm md:text-lg leading-tight">{item.title}</h4>
                  </div>
                  <div className="absolute top-3 right-3 bg-yellow-500 text-slate-900 p-2 rounded-full opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 shadow-lg hidden lg:block">
     ...
     {/* --- KAPCSOLAT --- */}
      <section id="kapcsolat" className="py-16 md:py-24 lg:py-32 bg-slate-50 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-slate-50 to-slate-200"></div>

        <div className="container mx-auto px-4 relative z-10">
          <RevealOnScroll>
            <div className="max-w-6xl mx-auto bg-white shadow-[0_10px_30px_rgba(0,0,0,0.1)] rounded-2xl md:rounded-3xl overflow-hidden flex flex-col md:flex-row border border-white">
              
              <div className="bg-slate-900 p-8 md:p-14 md:w-5/12 text-white relative overflow-hidden flex flex-col justify-between min-h-[400px]">
                <div className="relative z-10">
                  <h3 className="text-2xl md:text-3xl font-black uppercase italic mb-3 text-white">Kapcsolat</h3>
                  <p className="text-slate-400 mb-8 md:mb-12 text-sm md:text-base leading-relaxed">
                ...
                <div className="relative z-10 mt-8 md:mt-12 pt-6 border-t border-slate-800">
              ...
              <div className="p-8 md:p-14 md:w-7/12 bg-slate-50/50 flex flex-col justify-center relative">
                <div className="relative z-10">
                  ...
                  <p className="text-slate-600 mb-8 leading-relaxed font-medium text-sm md:text-base lg:text-lg">
                    A földmunka jellegéből adódóan a legpontosabb árajánlatot <strong>telefonon vagy helyszíni felmérés</strong> után tudjuk adni.
                  </p>
                  
                  <div className="flex items-center gap-4 mb-8 md:mb-10 p-4 md:p-6 bg-slate-900 rounded-xl md:rounded-2xl border border-slate-800 shadow-xl transition-transform hover:scale-[1.01]">
                    ...
                    <div>
                        <p className="text-white font-bold text-base md:text-xl leading-tight mb-0.5">Csali Tamás</p>
                        <p className="text-slate-400 text-xs md:text-sm font-medium uppercase tracking-wide">Tulajdonos, Gépkezelő</p>
                    </div>
                  </div>

                  <a href="tel:+36306457041" className="w-full group bg-gradient-to-r from-yellow-500 to-yellow-400 hover:from-yellow-400 hover:to-yellow-500 text-slate-900 font-black py-4 md:py-6 text-base md:text-xl uppercase tracking-wide transition-all shadow-xl hover:shadow-yellow-500/40 text-center flex items-center justify-center gap-3 rounded-xl transform hover:-translate-y-1 active:translate-y-0 active:scale-98">
                    <Phone size={24} className="group-hover:animate-pulse" /> Hívás most
                  </a>
                </div>
              </div>
      ...
      {/* --- FOOTER --- */}
      <footer className="bg-slate-950 py-10 md:py-16 text-slate-500 text-sm border-t border-slate-900">
        <div className="container mx-auto px-4 flex flex-col items-center text-center">
          ...
          <p className="mb-6 font-medium text-slate-400 max-w-md text-xs md:text-base">Balaton-felvidék • Földmunka • Fuvarozás</p>
          <p className="opacity-40 text-xs md:text-sm">&copy; {new Date().getFullYear()} Minden jog fenntartva.</p>
        </div>
      </footer>
  ...
  return (
    <RevealOnScroll>
      <div className="text-center mb-10 md:mb-16 lg:mb-20">
        <h2 className="text-yellow-500 font-black uppercase tracking-[0.2em] text-[10px] md:text-sm mb-2 md:mb-3 flex items-center justify-center gap-2">
          <span className="w-6 md:w-10 h-[2px] bg-yellow-500 inline-block"></span>
          {subtitle}
          <span className="w-6 md:w-10 h-[2px] bg-yellow-500 inline-block"></span>
        </h2>
...
function ServiceCard({ icon, title, desc, delay }: { icon: React.ReactNode, title: string, desc: string, delay: number }) {
  return (
    <RevealOnScroll delay={delay} className="h-full">
      <div className="group p-6 md:p-8 bg-white/80 backdrop-blur-sm hover:bg-white transition-all duration-500 border border-slate-200/60 hover:border-yellow-500 shadow-xl hover:shadow-2xl hover:shadow-yellow-500/10 rounded-2xl h-full flex flex-col relative overflow-hidden transform md:hover:-translate-y-2">
        <div className="absolute -right-10 -bottom-10 w-24 h-24 md:w-32 md:h-32 bg-yellow-500/5 rounded-full transition-transform group-hover:scale-150 duration-500"></div>
        ...
        <h3 className="text-lg md:text-2xl font-black text-slate-900 group-hover:text-slate-900 mb-2 md:mb-3 uppercase italic transition-colors">{title}</h3>
...
function ContactItem({ icon, title, content, href, highlight }: { icon: React.ReactNode, title: string, content: string, href?: string, highlight?: boolean }) {
  return (
    <div className="flex gap-4 md:gap-5 items-center group">
      <div className={`w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 flex items-center justify-center rounded-xl shrink-0 transition-all duration-300 shadow-lg ${highlight ? 'bg-yellow-500 text-slate-900 shadow-yellow-500/50 group-hover:scale-110' : 'bg-slate-800 text-yellow-500 group-hover:bg-yellow-500 group-hover:text-slate-900'}`}>
        {React.cloneElement(icon as React.ReactElement<{ className?: string }>, { className: "w-5 h-5 md:w-6 md:h-6 lg:w-7 lg:h-7" })}
      </div>
      <div className="min-w-0 flex-1">
        <div className="font-bold text-[10px] md:text-xs text-yellow-500/80 uppercase tracking-widest mb-0.5 md:mb-1">{title}</div>
        {href ? (
          <a href={href} className={`font-bold transition hover:text-yellow-500 block truncate ${highlight ? 'text-lg md:text-2xl lg:text-3xl text-white' : 'text-sm md:text-lg lg:text-xl text-slate-300'}`}>
            {content}
          </a>
        ) : (
          <div className="font-medium text-slate-300 text-sm md:text-lg lg:text-xl truncate">{content}</div>
        )}
      </div>
    </div>
  )
